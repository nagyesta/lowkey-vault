...
public class VaultBackupConfiguration {
    ...
    public SecretBackupRestoreController secretBackupRestoreController() {
        return new SecretBackupRestoreController(secretEntityToV72ModelConverter(), secretEntityToV72BackupConverter(), vaultService());
    }

    @Bean
    public SecretEntityToV72ModelConverter secretEntityToV72ModelConverter() {
        return new SecretEntityToV72ModelConverter(secretEntityToV72PropertiesModelConverter());
    }

    @Bean
    public SecretEntityToV72BackupConverter secretEntityToV72BackupConverter() {
        return new SecretEntityToV72BackupConverter(secretEntityToV72PropertiesModelConverter());
    }

    @Bean
    public SecretEntityToV72PropertiesModelConverter secretEntityToV72PropertiesModelConverter() {
        return new SecretEntityToV72PropertiesModelConverter();
    }

    @Bean
    public KeyBackupRestoreController keyBackupRestoreController() {
        return new KeyBackupRestoreController(keyEntityToV72ModelConverter(), keyEntityToV72BackupConverter(), vaultService(),
                keyRotationPolicyToV73ModelConverter(), keyRotationPolicyV73ModelToEntityConverter());
    }

    @Bean
    public KeyEntityToV72ModelConverter keyEntityToV72ModelConverter() {
        return new KeyEntityToV72ModelConverter(keyEntityToV72PropertiesModelConverter());
    }

    @Bean
    public KeyEntityToV72PropertiesModelConverter keyEntityToV72PropertiesModelConverter() {
        return new KeyEntityToV72PropertiesModelConverter();
    }

    @Bean
    public KeyEntityToV72BackupConverter keyEntityToV72BackupConverter() {
        return new KeyEntityToV72BackupConverter(keyEntityToV72PropertiesModelConverter());
    }

    @Bean
    public KeyRotationPolicyToV73ModelConverter keyRotationPolicyToV73ModelConverter() {
        return new KeyRotationPolicyToV73ModelConverter();
    }
