...
public class MissionOutlineDefinition extends LaunchAbortHook {
    ...
    protected Map<String, Consumer<AbortMissionCommandOps>> defineOutline() {
        return Map.of(SHARED_CONTEXT, ops -> {
            final var extractor = new TagDependencyNameExtractor();
            ...
            Stream.of("Key", "Secret", "Certificate").forEach(type -> {
                final var typeMatcher = matcher().dependencyWith(type).extractor(extractor).build();
                ...
                Stream.of("Create", "Get", "Delete", "List", "Update", "ListDeleted", "Recover", "Purge", "Backup", "Restore", "Alias")
                        .forEach(subtype -> {
                            final var subTypeMatcher = matcher().dependencyWith(type + subtype)
            ...
            Stream.of("CreateVault", "KeyRotate", "KeyImport", "KeyEncrypt", "KeySign", "CertificateImport",
                    ...
                    .forEach(tag -> {
                        final var matcher = matcher().dependencyWith(tag).extractor(extractor).build();
