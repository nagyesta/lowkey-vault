...
public class CommonAssertions {
    ...
    protected <T> void assertNull(@Nullable final T actual) {
    ...
    protected <K, V> void assertContainsEqualEntries(
            final Map<K, V> expected,
            final Map<K, V> actual) {
        assertEquals(expected.size(), actual.size());
        expected.forEach((key, value) -> assertEquals(value, actual.get(key)));
    }

    protected <E> void assertContainsEqualEntries(
            final Collection<E> expected,
            final Collection<E> actual) {
        assertEquals(expected, actual);
        assertEquals(expected.size(), actual.size());
    }

    protected <E extends Comparable<E>> void assertContainsEqualEntriesSorted(
            final Collection<E> expected,
            final Collection<E> actual) {
    ...
    protected void assertByteArrayLength(
